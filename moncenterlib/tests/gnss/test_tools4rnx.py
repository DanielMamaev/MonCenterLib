...
class TestTools4Rnx(TestCase):
    def setUp(self) -> None:
        ...
        self.path_data_raw = str(Path(__file__).resolve().parent) + "/data/tools4rnx/raw/"
        self.path_data_output = str(Path(__file__).resolve().parent) + "/data/tools4rnx/output/"

        try:
            os.mkdir(self.path_data_output)
        except FileExistsError:
            pass

    def clean_output(self):
        try:
            os.system(f"rm -r {self.path_data_output}*")
        except Exception:
            pass
    ...
    def test_scan_dir_raises(self):
        ...
        self.assertEqual(str(ms.exception), "The type of the 'input_dir' variable should be str")

        with self.assertRaises(TypeError) as ms:
            self.t4r.scan_dir('test', 123)
        self.assertEqual(str(ms.exception), "The type of the 'recursion' variable should be bool")
    ...
    def test_start_raises(self):
        ...
        self.assertEqual(str(ms.exception), "The type of the 'input_raw' variable should be 'str' or 'list'.")

        with self.assertRaises(TypeError) as ms:
            self.t4r.start('path', 123, self.t4r.get_default_config(), False, False)
        self.assertEqual(str(ms.exception), "The type of the 'output' variable should be str")

        with self.assertRaises(TypeError) as ms:
            self.t4r.start('path', 'path', 'dict()', False, False)
        self.assertEqual(str(ms.exception), "The type of the 'config' variable should be dict")
        ...
        with self.assertRaises(TypeError) as ms:
            self.t4r.start('path', 'path', self.t4r.get_default_config(), False, 'False')
        self.assertEqual(str(ms.exception), "The type of the 'show_progress' variable should be bool")
    ...
    def test_config(self, mock_subprocess):
        ...
        exp_cmd = [f'{str(Path(__file__).resolve().parent.parent.parent.parent)}/bin/RTKLIB-2.4.3-b34/app/consapp/convbin/gcc/convbin',
                   '-r', 'ubx',
                   '-v', '3.03',
