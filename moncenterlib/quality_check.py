...
class Anubis:
    ...
    def start_linux(self, obs, nav, input_logger=None):
        logger = input_logger
        ...
        cmd += f':inp:rinexo {obs} '
        cmd += f':inp:rinexn {nav} '
        cmd += ':qc:sec_sum=2 '
        cmd += ':qc:sec_hdr=0 '
        cmd += ':qc:sec_obs=1 '
        cmd += ':qc:sec_gap=1 '
        ...
        cmd += ':qc:sec_mpx=1 '
        cmd += ':qc:sec_snr=1 '
        cmd += f':out:xtr {obs}.xtr'
        logger.info('Запукс Anubis')
        os.system(cmd)
        try:
            f = open(f'{obs}.xtr', 'r')
            data = f.readlines()
            ...
            logger.error(
                f'Что то случилось с открытием файла Anubis {obs}.xtr. {e}')
            return False

        logger.info(f'Начинаем парсить файл Anubis {obs}.xtr')
        meta_data = {}
        flag_gnssum = False
            ...
            if '=TOTSUM' in row:
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.warning(f'Попался дефис. {e}')
        ...
        if flag_data_error:
            logger.error(f'Некорректные данные в файле Anubis {obs}.xtr.')
            return False
        ...
        try:
            ...
            logger.error(
                f'Что то случилось с формированием новой записи для Dataframe. {e}')
            return False
